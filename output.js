//Sun Jun 30 2024 07:17:30 GMT+0000 (Coordinated Universal Time)
...
async function main() {
  ...
  for (const item of KuGua) {
    ...
    // console.log(token)
    console.log("\u5F00\u59CB\u7B7E\u5230");
    let sign = await commonPost("/inflatedv3/popUpRedEnvelopes", {
      "type": 1,
      "invite_id": "",
      "code_ticket": "",
      "count": "",
    ...
    console.log("\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014");
    console.log("\u5F00\u59CB\u4EFB\u52A1");
    let taskList = await commonPost("/dailyTaskv2/index", {
      "token": token,
      "appid": appId,
      "openid": openId
    });
    for (let task of taskList.data) {
      ...
      if (task.status == 1) {
        ...
        let popUpRedEnvelopes = await commonPost("/inflatedv3/popUpRedEnvelopes", {
          "type": task.inflated_type,
          "token": token,
          "appid": appId,
          "openid": openId
        });
        ...
        let receiveRedEnvelopes = await commonPost("/inflatedv3/receiveRedEnvelopes", {
          "dialogId": dialogId,
          "token": token,
          "appid": appId,
          "openid": openId
    ...
    console.log("\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014");
    console.log("\u4F53\u73B0");
    let withdraw = await commonPost("/withdrawal/withdrawal", {
      "withdrawalId": 24,
      "token": token,
      "appid": appId,
      "openid": openId
    ...
    if (!amount) {
      let inflatedAmount = await commonPost("/inflatedv3/inflatedAmount", {
        "token": token,
        "appid": appId,
        "openid": openId
      });
