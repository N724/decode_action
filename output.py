...
class yuanshen():
 ...
 def read(self):
  ...
  while True:
   ...
   r=random.random()
   if jkey is None:
    url=f"http://{self.domain}/read_task/ddr?iu={self.iu}&type=7,7&t={r}"
   else:
    r=requests.get(f'http://{self.domain}/read_task/rd2',headers=self.h2)
    if r.status_code!=200:
     pass
    url=f"http://{self.domain}/read_task/ddr?iu={self.iu}&type=7,7&t={r}&jkey={jkey}"
 ...
 def main(self):
  ...
  self.getdoamin()
  print("======================")
  self.h2={"Host":f"{self.domain}","Connection":"keep-alive","User-Agent":ua,"X-Requested-With":"XMLHttpRequest","Accept":"*/*","Referer":f"http://{self.domain}/read_task/rd2/","Accept-Encoding":"gzip, deflate","Accept-Language":"zh-CN,zh;q=0.9,en-US;q=0.8,en;q=0.7","Cookie":f"PHPSESSID={self.cookie}"}
